...
export interface CollectionState<D> {
  loadKey: string | null;
  isLoading: boolean;
  isLoaded: boolean;
...
export default abstract class CollectionDuck<D> extends Duck<CollectionState<D>> {
  ...
  getLoadRequestedActionType() {
    return this.buildActionType(CollectionDuck.LOAD_REQUESTED);
  }

  getLoadSucceededActionType() {
    return this.buildActionType(CollectionDuck.LOAD_SUCCEEDED);
  }

  getLoadFailedActionType() {
    return this.buildActionType(CollectionDuck.LOAD_FAILED);
  }
